...
describe RubyRDF::Sesame do
  ...
  it "should execute SPARQL select query" do
    @graph.add(ex::a, ex::b, RubyRDF::PlainLiteralNode.new('test'))
    @graph.add(ex::d, ex::b, RubyRDF::PlainLiteralNode.new('test', 'en'))
    @graph.add(ex::d, ex::b, RubyRDF::TypedLiteralNode.new('test', ex::a.uri))
    @graph.add(ex::d, ex::b, ex::e)
    @graph.add(ex::d, ex::b, RubyRDF::BlankNode.new)

    result = @graph.select("SELECT ?y WHERE {?x #{ex::b} ?y . }")
    result.size.should == 5
    result.any?{|r| r['y'] == RubyRDF::PlainLiteralNode.new('test')}.should be_true
    result.any?{|r| r['y'] == RubyRDF::PlainLiteralNode.new('test', 'en')}.should be_true
    result.any?{|r| r['y'] == RubyRDF::TypedLiteralNode.new('test', ex::a.uri)}.should be_true
  ...
  it "should commit a transaction" do
    stmt1 = RubyRDF::Statement.new(ex::sub, ex::pred1, RubyRDF::PlainLiteralNode.new('plainLiteral', 'en'))
    stmt2 = RubyRDF::Statement.new(RubyRDF::BlankNode.new("test2"), ex::pred2, RubyRDF::TypedLiteralNode.new('plainLiteral', ex::datatype))
    @graph.add(stmt1)
